/*
  ...
  Include 2 power sources: AA(1.2V-1.5V)*4 for motor drive board, 9V*1 for auduino uno r3
...
const unsigned long MAX_DRIVE_DURATION_FREE = 3000; //milliseconds,the MAX driving time to any directions under free driving mode
...
void drivingReset() {
  ...
  driveStartTime = now;				//reset the driving start time
  ...
  if (obstacleState == B0000) {
    ...
    if (driveMode == DRIVE_MODE_OBSTACLE) {
      if (now - driveStartTime >= driveDuration) {//obstacle avoidance driving over time
        ...
        if (driveDirection != chooseDriveDirection(obstacleState)) { //the new driving direction not equals the old
          drivingReset();	//reset the driving based on the obstacleState
        } else {	////the new driving direction equals the old
          //do nothing
        }
      }
    } else  { //obstacles detectd under free dring mode
